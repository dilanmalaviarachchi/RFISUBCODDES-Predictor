...
# Sample average values (replace with your real data if needed)
avg_count = 5.0
avg_rpc = 10.0
...
with col1:
    ...
    with st.container():
        ...
        point_of_sale = st.selectbox("Point of Sale", le_pos.classes_, key="pos_select")
        year = st.number_input("Year", min_value=2015, max_value=2030, step=1, key="year_input")
        month = st.selectbox("Month", list(range(1, 13)), key="month_select")
...
if 'predictions' in st.session_state:
    ...
    with col2:
        ...
        tab1, tab2, tab3 = st.tabs(["Probability Distribution", "Top Categories", "Monthly Trend"])
...
with st.sidebar:
    st.markdown("## 📘 About")
    st.markdown("""
This predictive tool helps forecast the most likely ancillary revenue types based on:
...
""")
    ...
    st.write(f"**Point of Sale Options:** {len(le_pos.classes_)}")
